...
<body class="min-h-screen flex flex-col" style="background-color: #050505 !important; color: #ffffff !important;">
    ...
    <?php if (isset($pdo)): ?>
        <?php
        $stmtPending = $pdo->prepare("SELECT id FROM pedidos WHERE id_usuario = ? AND estado = 'pendiente' ORDER BY fecha DESC LIMIT 1");
        $stmtPending->execute([$_SESSION['usuario']['id']]);
        $pendingOrder = $stmtPending->fetchColumn();
        ?>
        <?php if ($pendingOrder && $current !== 'checkout.php'): ?>
            <div style="background-color: #ffaa00; color: #111; text-align: center; padding: 12px 20px; font-weight: 500; font-size: 0.95rem; z-index: 1000; position: relative; border-bottom: 1px solid #cc8800; display: flex; align-items: center; justify-content: center; gap: 10px; flex-wrap: wrap;">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"></path><line x1="12" y1="9" x2="12" y2="13"></line><line x1="12" y1="17" x2="12.01" y2="17"></line></svg>
                <span>¡Tienes una compra pendiente de pago!</span>
                <a href="checkout.php?pay_order=<?php echo $pendingOrder; ?>" style="background-color: #111; color: #fff; text-decoration: none; font-weight: 600; padding: 6px 14px; border-radius: 4px; font-size: 0.85rem; transition: background 0.2s;" onmouseover="this.style.backgroundColor='#333'" onmouseout="this.style.backgroundColor='#111'">Continuar con el pago</a> 
            </div>
        <?php endif; ?>
    <?php endif; ?>
