...
<body class="min-h-screen flex flex-col">
    <header class="py-4 px-4 grid grid-cols-3 items-center">
        <!-- Logo a la izquierda -->
        ...
        <nav class="flex justify-center">
            <ul class="flex gap-6 text-lg font-semibold">
                <li>
                    <a href="index.php"
                        class="main-menu-link flex items-center gap-2 <?php if ($current == 'index.php')
                            echo 'active-menu'; ?>">
                ...
                <li>
                    <a href="productos.php"
                        class="main-menu-link flex items-center gap-2 <?php if ($current == 'productos.php')
                            echo 'active-menu'; ?>">
                ...
                <li>
                    <a href="servicios.php"
                        class="main-menu-link flex items-center gap-2 <?php if ($current == 'servicios.php')
                            echo 'active-menu'; ?>">
                ...
                <li>
                    <a href="contacto.php"
                        class="main-menu-link flex items-center gap-2 <?php if ($current == 'contacto.php')
                            echo 'active-menu'; ?>">
        ...
        <div class="flex items-center justify-end gap-4">
            <a href="carrito.php"
                class="relative flex items-center gap-2 hover:text-red-500 transition px-3 py-2 rounded-lg hover:bg-white/5"
                aria-label="Carrito">
            ...
            <?php if (isset($_SESSION['usuario']) && is_array($_SESSION['usuario'])): ?>
                <div class="relative" id="user-menu-container">
                    <button id="user-menu-button"
                        class="hover:text-red-500 transition flex items-center gap-2 p-2 rounded-full hover:bg-[#333]">
                        <?php if (!empty($_SESSION['usuario']['foto'])): ?>
                            <img src="<?php echo htmlspecialchars($_SESSION['usuario']['foto']); ?>" alt="Foto de perfil"
                                class="w-8 h-8 rounded-full object-cover border-2 border-[#333]">
                        <?php else: ?>
                            <div
                                class="w-8 h-8 rounded-full bg-red-600 flex items-center justify-center text-white font-bold text-sm">
                                <?php echo strtoupper(substr($_SESSION['usuario']['nombre'], 0, 1)); ?>
                            </div>
                        <?php endif; ?>
                        <i data-lucide="chevron-down" class="w-5 h-5 text-white user-chevron" aria-hidden="true"></i>
                    </button>
                    <div id="user-menu-dropdown"
                        ...
                        <div class="px-4 py-2 border-b border-[#333]">
                            <div class="font-semibold text-white user-name">
                                <?php echo htmlspecialchars($_SESSION['usuario']['nombre']); ?></div>
                            <div class="text-sm text-gray-400 user-email">
                        ...
                        <?php if ($_SESSION['usuario']['rol'] === 'admin'): ?>
                            <a href="admin/dashboard.php" class="block px-4 py-2 hover:bg-[#181818] transition text-white">
            ...
            <?php else: ?>
                <a href="#" onclick="abrirModalLogin(); return false;" id="btn-login-header"
                    class="hover:text-red-500 transition flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-white/5">
    ...
    <script>
        ...
        if (userMenuButton && userMenuDropdown) {
            userMenuButton.addEventListener('click', function (e) {
                ...
                userMenuButton.classList.toggle('open', isOpen);
            });
